...
// ---------------------------------------------------------------
// AUTOCOMPLETE WITH PERSISTENT DROPDOWN
// ---------------------------------------------------------------
...
inputEl.addEventListener("input", function () {
    ...
    // delay για αποφυγή spam στο API
    debounceTimer = setTimeout(() => {

        let url =
            "https://nominatim.openstreetmap.org/search?" +
            "format=json&addressdetails=1&bounded=1" +
            "&limit=10" +
            "&viewbox=32.00,35.80,34.90,34.50" +
            "&q=" + encodeURIComponent(query);

        fetch(url, {
            headers: { "User-Agent": "CyprusRideApp/1.0 (contact@example.com)" }
        })
            .then(res => res.json())
            .then(data => {
